...
function Detailspage() {
  ...
  const { data } = ApiHook();
console.log(details)
  const { description, status,area,location, estate_title,  image, facilities, price } =details  || {};
  useEffect(() => {
    if (data) {
  ...
  return (
    <div>
      ...
      {description}
          ...
          <p className="text-lg"><span className="font-bold">status : </span>{status}</p>
            <p className="text-lg"><span className="font-bold">Area :</span> {area}</p>
            ...
            <ul style={listItemStyle} className="list-disc list-inside mb-4 bg-gray-100 p-4 rounded-lg">
            {facilities &&
                      facilities.map((val, index) => (
                        <li
                          key={index}
                          className="p-2 rounded-lg lg:font-semibold "
                        >
                          {val}
                        </li>
                      ))}
